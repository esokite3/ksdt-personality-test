  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="text-center mb-10">
        ...
        >
          <Radio className="w-16 h-16 text-[#ff8dca] drop-shadow-[0_0_20px_rgba(255,141,202,0.6)]" />
        </motion.div>
        <motion.h1 
      ...
      >
        <h3 className="text-2xl font-black mb-6 uppercase tracking-wide">Other Great Matches</h3>
          ...
          {allDepartments.slice(1, 4).map((dept, index) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="font-black uppercase tracking-wide mb-1">{dept.name}</h4>
                  <p className="text-sm text-gray-400">{dept.description}</p>
                </div>
              </div>
              <div className="text-right">
                <div className="text-2xl font-black text-[#ff8dca]">{dept.score}%</div>
              </div>
            </motion.div>
          ))}
        </div>
            ...
            {allDepartments.map((dept, index) => (
              <div key={index} className="flex items-center justify-between py-2 border-b border-white/10 last:border-0">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-bold uppercase tracking-wide">{dept.name}</span>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-black w-12 text-right text-[#ff8dca]">{dept.score}%</span>
                </div>
              </div>
            ))}
          </div>
